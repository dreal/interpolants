...
Figure~\ref{fig:ode} shows an interpolant obtained by for the following system of ODEs:
\begin{eqnarray*}
A: &~~~~&  x_t = x₀ + \int_0^t \! -x \, \mathrm{d}x ∧ x₀ = 3 \\
B: &~~~~&  y_t = y₀ + \int_0^t \! -y \, \mathrm{d}y ∧ y₀ = 2 ∧ x_t = y_t
\end{eqnarray*}

\todo[inline]{a more interesting example}

\begin{figure}
\centering
\includegraphics[scale=0.6]{img/ode.pdf}
\caption{
    ToDo ...
}
\label{fig:ode}
