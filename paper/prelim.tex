...
\inferrule{
  x_i ∈ [l_i, p] ∧ \bigwedge_{j ≠ i} x_j ∈ D_j ∧ C ⇒ ⊥ \\
  x_i ∈ [p, u_i] ∧ \bigwedge_{j ≠ i} x_j ∈ D_j ∧ C ⇒ ⊥
}{
...
\todo[inline,color=green]{comparing the above and below, which do you like?}
\begin{mathpar}
\inferrule{ {} }{
  \neg \Big(\vec x ∈ \vec D ∧ c\Big)
}{(\thLem)}\\
...
\inferrule{
  \neg\Big(x_i ∈ [l_i, p] ∧ \bigwedge_{j ≠ i} x_j ∈ D_j ∧ C\Big) \\
 \neg\Big(x_i ∈ [p, u_i] ∧ \bigwedge_{j ≠ i} x_j ∈ D_j ∧ C\Big)
}{
  \neg\Big(x_i\in [l_i, u_i] \wedge \bigwedge_{j\neq i} x_j∈\vec D_j ∧ C\Big)
}{(\splt)}
\end{mathpar}
