...
\begin{mathpar}
\inferrule{ {} }{
  \vec x ∈ \vec D ∧ c ⇒ ⊥ \quad [l(f) ≠ \textsc{a}]
}{(\thLemI)}\\

\inferrule{
  C = c ∧ \bigwedge_k C_k \\
  \vec x ∈ \vec D ∧ c ⇒ ⊥ \quad [I]
}{
  \vec x ∈ \vec D ∧ C ⇒ ⊥ \quad [I]
}{(\weakenI)}\\
...
\inferrule{
  x_i ∈ [l_i, p] ∧ \bigwedge_{j ≠ i} x_j ∈ D_j ∧ C ⇒ ⊥ \quad [I₁] \quad\quad
  x_i ∈ [p, u_i] ∧ \bigwedge_{j ≠ i} x_j ∈ D_j ∧ C ⇒ ⊥ \quad [I₂] 
}{
  x_i\in [l_i, u_i]\wedge \bigwedge_{j\neq i} x_j ∈ \vec D_j ∧ C ⇒ ⊥ \quad
  \left[ \substack{ I₁ ∨ I₂     \qquad \quad ~~  \text{if} ~ l(x_i) = \textsc{a} \\
                    ite(x_i < p, I₁, I₂) ~~ \text{if} ~ l(x_i) = \textsc{ab}\\
                    I₁ ∧ I₂     \qquad \quad ~~  \text{if} ~ l(x_i) = \textsc{b}}\right]
}{(\spltI)}
